...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  GlobalKey<State> _keyLoader355 =
      new GlobalKey<State>(debugLabel: 't4eterterterqeqe');
  GlobalKey<State> _keyLoader356 =
      new GlobalKey<State>(debugLabel: 't4eterterterqeqe');
  ...
  Future uploadFileWithProgressIndicator(
    ...
    if (isthumbnail == false && isVideo(pickedFile!.path) == true) {
      ShowLoading().open(context: this.context, key: _keyLoader356);
      try {
      ...
      ShowLoading().close(context: this.context, key: _keyLoader356);
...
//-pprefs
  ...
  Future uploadSelectedLocalFileWithProgressIndicator(BuildContext context,
      File file, bool isVideo, bool isthumbnail, int timeEpoch,
      {String? filenameoptional}) async {
    var finalFile = file;

    if (IsVideoQualityCompress == true && isVideo == true) {
      ShowLoading().open(context: context, key: _keyLoader355);
      try {
        await compress.VideoCompress.setLogLevel(0);
      ...
      ShowLoading().close(context: context, key: _keyLoader355);
    } else {
      finalFile = file;
    }

    String ext = p.extension(finalFile.path);
    ...
    UploadTask uploading = reference.putFile(finalFile);
    ...
    if (isthumbnail == true) {
      ...
      await _mediaInfo.getMediaInfo(finalFile.path).then((mediaInfo) {
        setStateIfMounted(() {
          videometadata = jsonEncode({
            "width": mediaInfo['width'],
            "height": mediaInfo['height'],
                                                                          ...
                                                                          new AllinOneCameraGalleryImageVideoPicker(
                                                                            ...
                                                                            onTakeFile: (file,
                                                                              ...
                                                                              if (isVideo == true) {
                                                                                ...
                                                                                String? videoUrl = await uploadSelectedLocalFileWithProgressIndicator(this.context, file, true, false, timeStamp, filenameoptional: videofileName);
                                                                                if (videoUrl != null) {
                                                                                  String? thumnailUrl = await uploadSelectedLocalFileWithProgressIndicator(this.context, thumnail!, false, true, timeStamp);
                                                                                  if (thumnailUrl != null) {
                                                                                ...
                                                                                String? url = await uploadSelectedLocalFileWithProgressIndicator(this.context, file, false, false, timeStamp, filenameoptional: imagefileName);
                                                                                if (url != null) {
      ...
      if (docs.docs.isNotEmpty) {
        ...
        for (final doc in docs.docs) {
          ...
          try {
            ...
            // debugPrint(_doc[Dbkeys.content].toString());
