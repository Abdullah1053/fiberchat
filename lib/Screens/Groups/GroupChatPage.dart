...
class _GroupChatPageState extends State<GroupChatPage>
  ...
  GlobalKey<State> _keyLoader22424 =
      new GlobalKey<State>(debugLabel: 'qqqeqeqsfdfgdgfssaadqeqe');

  GlobalKey<State> _keyLoader22426 =
      new GlobalKey<State>(debugLabel: 'qqqeqeqsfdfgdgfssaadqeqe');
...
//       //----sharing intent action:
  ...
  Future uploadFileWithProgressIndicator(
    ...
    if (isthumbnail == false && isVideo(pickedFile!.path) == true) {
      ShowLoading().open(context: this.context, key: _keyLoader22424);
      try {
      ...
      ShowLoading().close(context: this.context, key: _keyLoader22424);
  ...
  Future uploadSelectedLocalFileWithProgressIndicator(
    ...
    if (IsVideoQualityCompress == true && isVideo == true) {
      ShowLoading().open(context: this.context, key: _keyLoader22426);
      try {
        await compress.VideoCompress.setLogLevel(0);
      ...
      ShowLoading().close(context: this.context, key: _keyLoader22426);
    } else {
      finalFile = file;
    }
